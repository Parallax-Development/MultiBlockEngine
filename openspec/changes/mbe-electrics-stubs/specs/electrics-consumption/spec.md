...
The system SHALL provide consumer machines (e.g., Electric Furnace) that pull energy from the `energy` network graph to process items or provide utility. The energy SHALL ONLY be pulled when there is an active valid recipe and space for output, and SHALL be drawn by querying connected network nodes.
