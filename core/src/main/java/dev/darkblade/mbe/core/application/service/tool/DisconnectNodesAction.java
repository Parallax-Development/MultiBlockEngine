...
public final class DisconnectNodesAction implements ToolAction {
    ...
    public DisconnectNodesAction(NetworkService networkService, ToolSessionService sessionService, ToolModeContextResolver contextResolver) {
    ...
    @Override
    public ActionId id() {
        return WireCutterActions.DISCONNECT;
    }
    ...
    public WrenchResult execute(WrenchContext context) {
        ...
        Optional<Map<String, Object>> session = sessionService.get(playerId, "disconnect_nodes");
        if (session.isEmpty()) {
            sessionService.put(playerId, "disconnect_nodes", Map.of("from", serialize(current.position()), "type", current.type().id()));
        ...
        sessionService.clear(playerId, "disconnect_nodes");
        if (from.isEmpty()) {
            return WrenchResult.fail("core.wrench.not_found");
        }
